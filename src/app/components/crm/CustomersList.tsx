...
const statusBadge = (s?: string) => {
  if (s === 'Active') return pill(s, 'good');
  if (s === 'Repeat') return pill(s, 'good');
  if (s === 'Lead') return pill(s, 'pending');
  if (s === 'Inactive') return pill(s, 'bad');
              ...
              : filtered.map(c => (
                <tr key={c.id}>
                  ...
                  {isAdmin && <td style={{ ...S.td, textAlign: 'right' }}>
                    <button title="Edit" style={S.rowBtn} onClick={() => { setEditing(c); setShowModal(true); }}><Pencil size={13} /></button>
